...
export async function getAdminAffiliateDetail(id: string) {
  ...
  const byAffiliate = await supabase.from('affiliates').select('id,profile_id,affiliate_code,status,commission_default,created_at,profiles(full_name,phone,country,status,created_at)').eq('id', id).maybeSingle();
  affiliate = byAffiliate.data;
  if (!affiliate) {
    const byProfile = await supabase.from('affiliates').select('id,profile_id,affiliate_code,status,commission_default,created_at,profiles(full_name,phone,country,status,created_at)').eq('profile_id', id).maybeSingle();
    affiliate = byProfile.data;
  }
  if (!affiliate) return null;
  affiliate.default_commission = affiliate.commission_default;
