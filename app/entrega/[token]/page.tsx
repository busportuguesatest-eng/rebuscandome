...
export default async function DeliveryPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (/^[A-Za-z0-9_-]{32,160}$/.test(clean)) {
    ...
    if (access) {
      ...
      const { data: rows } = await service.from('product_assets').select('id,file_name,original_name,storage_path,mime_type,asset_type,position,metadata').eq('product_id', access.product_id).eq('status','active').in('asset_type',['ebook','delivery','bonus']).order('position', { ascending: true });
      product = p;
      for (const row of rows ?? []) {
        if ((row as any).metadata?.delivery_kind === 'url' && /^https:\/\//i.test(String((row as any).metadata?.url || ''))) assets.push({ ...row, signed_url: (row as any).metadata.url });
        else {
  ...
  return <main className="payment-result-page delivery-page">
    <div className="payment-result-card is-paid">
      ...
      {valid && product ? <>
        ...
        <div className="delivery-assets">{assets.length ? assets.map(asset => { const isWeb = /^https:\/\//i.test(String(asset.signed_url || '')); return <a key={asset.id} href={asset.signed_url} target="_blank" rel="noreferrer" className="payment-primary-link"><span>{isWeb ? '🌐' : asset.asset_type === 'ebook' ? '📘' : '📦'}</span> {isWeb ? 'Abrir página web' : 'Descargar'} {asset.original_name || asset.file_name}</a>; }) : <p>No hay recursos de entrega publicados todavía. Contacta al equipo de Rebuscándome.</p>}</div>
      </> : <>
        <div className="payment-result-icon" aria-hidden="true">!</div>
