...
export default async function AdminProductDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: productAssets } = await supabase.from('product_assets').select('id,asset_type,storage_path,original_name,file_size,metadata').eq('product_id', product.id).eq('status','active');
  const resourceCount = (productAssets ?? []).filter((a:any) => !['cover','gallery','thumbnail'].includes(a.asset_type)).length;
  const deliveryAssets = (productAssets ?? []).filter((a:any) => ['delivery','ebook','bonus'].includes(a.asset_type)).map((a:any)=>({id:a.id,original_name:a.original_name,asset_type:a.asset_type,file_size:a.file_size ?? null,metadata:a.metadata ?? null}));
