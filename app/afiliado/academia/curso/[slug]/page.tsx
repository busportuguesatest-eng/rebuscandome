...
export default async function CourseRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: progress } = lessonIds.length ? await supabase.from('lesson_progress').select('lesson_id,completed,completed_at').eq('user_id', user.id).in('lesson_id', lessonIds) : { data: [] as any[] };
  const completed = (progress || []).filter((p:any)=>Boolean(p.completed) || Boolean(p.completed_at)).map((p:any)=>p.lesson_id);
