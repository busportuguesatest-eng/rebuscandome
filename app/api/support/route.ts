...
export async function POST(request: Request) {
  ...
  if (actor.profile.role === 'affiliate') {
    ...
    const { count: priorAffiliateMessages } = await service.from('support_messages').select('id', { count: 'exact', head: true }).eq('thread_id', thread.id).eq('sender_role', 'affiliate');
    ...
    if ((priorAffiliateMessages ?? 0) === 0) await service.from('support_messages').insert({ thread_id: thread.id, sender_profile_id: null, sender_role: 'system', body: AUTO_GREETING });
