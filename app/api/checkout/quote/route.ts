...
function fallbackPayment(rows:any[]|null){
  const row=rows?.[0];
  return row ?? {method:'pago_movil',enabled:true,bank_name:process.env.MANUAL_PAYMENT_BANK_NAME?.trim()||'',account:'',account_type:'',holder:process.env.MANUAL_PAYMENT_HOLDER?.trim()||'',identifier:process.env.MANUAL_PAYMENT_IDENTIFIER?.trim()||'',phone:process.env.MANUAL_PAYMENT_PHONE?.trim()||''};
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const countryCode=(request.headers.get('x-vercel-ip-country')||'').toUpperCase().slice(0,2)||'UN';
    const { data: paymentRows } = await service.from('payment_method_settings').select('method,enabled,bank_name,account,account_type,holder,identifier,phone').eq('method','pago_movil');
    const manual=fallbackPayment(paymentRows);
    const paypalEnabled=Boolean(process.env.PAYPAL_CLIENT_ID?.trim());
    const isVenezuela=countryCode==='VE';
    const payment={
      country_code:countryCode,
      paypal:{method:'paypal',enabled:paypalEnabled},
      pago_movil:{...manual,enabled:manual.enabled&&isVenezuela},
    };
    return NextResponse.json({ok:true,quote:{id:quote.id,product_id:quote.product_id,affiliate_id:quote.affiliate_id,price_usd:Number(quote.price_usd),exchange_rate:Number(quote.exchange_rate),amount_ves:Number(quote.amount_ves),rate_source:quote.rate_source,rate_fetched_at:quote.rate_fetched_at,expires_at:quote.expires_at,country_code:countryCode,is_venezuela:isVenezuela,product:{name:product.name,short_description:product.short_description,cover_url:coverUrl},payment}}, {status:201});
