...
export async function POST(request: Request) {
  ...
  try {
    ...
    const allowedManual = method === 'pago_movil';
    ...
    const { data: rawOrder, error: orderError } = await service.rpc('create_pending_order_from_quote', {
      ...
      p_customer_country: (request.headers.get('x-vercel-ip-country') || 'UN').toUpperCase().slice(0,2),
    }).single();
    ...
    if (allowedManual) {
      ...
      const { data: settingsRow } = await service.from('payment_method_settings').select('method,enabled,bank_name,account,account_type,holder,identifier,phone').eq('method', 'pago_movil').eq('enabled', true).maybeSingle();
      const settings = settingsRow ?? {
        method, enabled:true,
        bank_name: process.env.MANUAL_PAYMENT_BANK_NAME?.trim() || '', account:'', account_type:'', holder:process.env.MANUAL_PAYMENT_HOLDER?.trim()||'', identifier:process.env.MANUAL_PAYMENT_IDENTIFIER?.trim()||'', phone:process.env.MANUAL_PAYMENT_PHONE?.trim()||''
      };
      const commonReady=Boolean(settings.bank_name && settings.identifier);
      const methodReady=Boolean(settings.phone);
