...
export async function POST(request: Request) {
  ...
  const method = body.method === 'pago_movil' ? 'pago_movil' : null;
  ...
  if (row.enabled && (!commonReady || !specificReady)) return NextResponse.json({ ok:false, message:'Completa Banco, Cédula/RIF y Teléfono antes de activar Pago Móvil.' }, { status:400 });
